...
def create_config(project_name, project_label, pipeline, wiki):
    ...
    "properties": {
      ...
      "initial_mapping": {
        ...
        "mappings": [
          ...
          {"source": "source_type", "target": "Wiki", "operation": "set"},
  ...
  config['properties']['excludeRegexes'] = [".*=login.*", ".*login.action.*", ".*signup.action.*"]
  if "excludes" in wiki:
    config['properties']['excludeRegexes'].append(wiki["excludes"])
  schedule = None
  if "schedule" in wiki:
